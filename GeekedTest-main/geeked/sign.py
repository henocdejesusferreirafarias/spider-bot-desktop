...
class Signer:
    ...
    @staticmethod
    def _human_passtime(base: float = 600, per_unit: float = 0.0, spread: float = 150) -> int:
        """Tempo de resolucao (ms) com distribuicao gaussiana truncada para imitar
        um humano. `base` + `per_unit` define o centro; `spread` o desvio padrao."""
        center = base + per_unit
        value = random.gauss(center, spread)
        return int(max(280, min(4500, value)))
...
function encrypt_asymmetric_2(input, key) {
    ...
    def generate_pow(lot_number_pow, captcha_id_pow, hash_func, hash_version, bits, date, empty) -> dict:
        ...
        while True:
            ...
            if not hashed_value.startswith(prefix):
                continue

            if bit_remainder == 0:
                return {'pow_msg': pow_string + h, 'pow_sign': hashed_value}

            threshold = {1: 7, 2: 3, 3: 1}[bit_remainder]
            if int(hashed_value[bit_division], 16) <= threshold:
                return {'pow_msg': pow_string + h, 'pow_sign': hashed_value}
    ...
    def generate_w(data: dict, captcha_id: str, risk_type: str):
        ...
        if risk_type in ("ai", "invisible"):
            ...
            base |= {
                # Tempo de arraste correlacionado com a distancia (~1.6px/ms) + ruido,
                # em vez de um uniforme fixo facil de detectar estatisticamente.
                "passtime": Signer._human_passtime(base=320, per_unit=left * 1.6, spread=140),
                "setLeft": left,
            ...
            base |= {
                "passtime": Signer._human_passtime(base=900, spread=350),
                "userresponse": IconSolver(data["imgs"], data["ques"]).find_icon_position()
            }
        elif risk_type == "nine":
            base |= {
                "passtime": Signer._human_passtime(base=1000, spread=400),
                "userresponse": NineSolver(data["imgs"], data["ques"], data.get("nine_nums", 3)).find_icon_positions()
            }
        else:
